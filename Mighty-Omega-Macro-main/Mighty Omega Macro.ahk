...
If !FileExist("creamLib") {
    MsgBox, the data file "creamLib" folder is missing`nExtract file.
    ExitApp
}
whr := ComObjCreate("WinHttp.WinHttpRequest.5.1")
whr.Open("GET", "https://pastebin.com/raw/BSYvTnMQ", False), whr.Send(), whr.WaitForResponse()
v := whr.ResponseText
if (v > Version) {
    MsgBox, 51, Update Check, A new version of Vivace's Macro is avalable.`n`nCurrent Version: %Version%`nLastest Version: %v%`n`nVisit download page?
    ifMsgBox, Yes
    {
...
Gui, 2:Show, w216 h68, Vivace's Status Window
OnMessage(0x200,"MOUSEOVER")
WinWait, Vivace's Status Window
WinGetPos,,, W, H, Vivace's Status Window
WinGetPos,,,, _h_, ahk_class Shell_TrayWnd
ArrayPosition := [(A_ScreenWidth)-W, (A_ScreenHeight)-(H+_h_)]
WinMove, Vivace's Status Window,, ArrayPosition[1], ArrayPosition[2]
global Status
Notify(NewMessage) {
    Status.= NewMessage "`n"
    ...
    case "Missing" : {
        ...
        If (!UserID) or (!Webhook) {
            ...
            If (GetUrlStatus(Webhook, 10) != 200) {
                ...
                Msgbox, 4, Vivace's Macro, Do you want to disable webhook and Continue the macro?
                IfMsgBox, Yes
                {
                ...
                whr := ComObjCreate("WinHttp.WinHttpRequest.5.1")
                post={"username":"i love vivace's macro","content":"%UserID% hewwo"}
                whr.Open("POST", Webhook, False),whr.SetRequestHeader("Content-Type", "application/json"),whr.Send(post)
                Notify("Posted")
                MsgBox,4, Vivace's Macro, Do you get Ping? / Is it Correctly Ping Your Name
                IfMsgBox, No 
                {
                    Reload
                }
...
Gui, Add, Text,x0 y0 w700 h30 +BackgroundTrans Center gMove, Version: %Version%  Version: %A_AhkVersion%
Gui, Add, Text,ym xm,Vivace's Macro
Gui, Add, GroupBox, xm ym+20 w30 h260
Gui, Add, GroupBox, xm+50 ym+20 w411 h260 vTitle, Home
Gui, Add, Text, xm+10 ym+40 gBack, <
Gui, Add, Picture,w27 h30 xm+1 ym+65          gTabTreadmill, creamLib\Icon&Menu\treadmill.png
Gui, Add, Picture,w27 h30 xm+1 ym+98          gTabWeight, creamLib\Icon&Menu\weight.png
Gui, Add, Picture,w27 h30 xm+1 ym+131         gTabSP, creamLib\Icon&Menu\sp.png
Gui, Add, Picture,w27 h30 xm+1 ym+164         gTabSS, creamLib\Icon&Menu\ss.png
Gui, Add, Picture,w27 h30 xm+1 ym+197         gTabDura, creamLib\Icon&Menu\heart.png
Gui, Add, Picture,w27 h30 xm+1 ym+230         gTaball41, creamLib\Icon&Menu\dot.png
Gui, Margin, 80, 50
...
Gui, Add, DDL, w150 ym+110 xm    vT6            ,Inventory|1-0|Strave
...
Gui, Add, DDL, w150 ym+65 xm     vW4            ,Inventory|1-0|Protein+Inventory|Strave
...
Treadmill:
    ...
    If (T14 = "Video") {
        ...
        Gui, 3:Show,, Vivace's Recording
        WinWaitClose, Vivace's Recording
    }
    ...
    If (T16 = "SetStartDelay") or (T16 = "Both") or (T16 = "SetWaitDelay") {
        ...
        Gui, 4:Show,, Vivace's ExtraGain
        WinWaitClose, Vivace's ExtraGain
    } else {
        Delay1 := 9000
    }
    if (!Delay1) {
    ...
    Loop,
    {
        ...
        Loop,
        {
            Loop, Parse, Button, `,
            {
                ImageSearch,,, 200, 240, 600, 300, *50 creamLib\TrainingIcon\%A_LoopField%.bmp
                If (ErrorLevel = 0) {
            ...
            If (W4 = "Protein+Inventory") {
                ...
                If (ErrorLevel = 1) {
                    ...
                    If (ErrorLevel = 1) { 
                        ...
                        Ping("Protein Ranout, Continue Macroing")
                        W4 := "Inventory"
                    } else {
                        Click
    ...
    Loop,
    {
        ...
        If (SP4 != "Strave") {
            If (SP4 = "Scalar+Inventory") {
                i := "3-0"
                Notify("Searching for status")
                ImageSearch,,, 0, 145, 70, 170, *10 creamLib\BasicUI\DrinkStatus2.bmp
                If (ErrorLevel = 1) {
                    Notify("status not found")
...
SS:
    ...
    If (SS4 = "Custom") {
        if (path1) {
            InputBox, path1, Vivace's Macro,%SS2% 1st Custom Walk Distance (Default: %path1%ms),, 400, 130
            if (ErrorLevel = 1) {
        ...
        if (path2) {
            InputBox, path2, Vivace's Macro,%SS2% 2nd Custom Walk Distance (Default: %path2%ms),, 400, 130
            If (ErrorLevel = 1) {
                MsgBox,,Viavce's Macro, Missing infomation 
                ExitApp
            }
    ...
    Loop,
    {
        ...
        ImageSearch,,, 60, 525, 750, 590, *40 creamLib\BasicUI\Speed.bmp
        If (ErrorLevel = 0) {
            ...
            Loop,
            {
                ...
                Sleep 1000
                ImageSearch,,, 65, 525, 750, 585, *40 creamLib\BasicUI\Speed.bmp
...
Dura:
    ...
    MsgBox, 262144,Vivace's Macro,Select account for left side
    IfMsgBox, Ok
    {
        Global LeftPID := Resize("Left")
    }
    MsgBox, 262144,Vivace's Macro,Select account for right side
    IfMsgBox, Ok
    {
    ...
    Loop
    {
        ...
        ImageSearch,,, 10, 90, 260, 120, *30 creamLib\BasicUI\HPBar.bmp
        If (ErrorLevel = 0) {
            f(LeftPID)
            Sleep 3000
            ImageSearch,,, 50, 515, 735, 580, *30 *Trans0x00FF00 creamLib\BasicUI\Dura.bmp
            If (ErrorLevel = 1) {
                Timer := A_TickCount
                Loop
                {
        ...
        if (d2 = "Both") {
            CoordMode, Pixel, Screen
            ImageSearch,,, 810, 90, 1060, 120, *30 creamLib\BasicUI\HPBar.bmp
            If (ErrorLevel = 0) {
                f(RightPID)
                Sleep 3000
                ImageSearch,,, 850, 515, 1535, 580, *30 *Trans0x00FF00 creamLib\BasicUI\Dura.bmp
                If (ErrorLevel = 1) {
                    Timer := A_TickCount
                    Loop
                    {
...
Eat(i, v) {
    ...
    Loop
    {
        ...
        If (ErrorLevel = 1) { 
            If (v = "Inventory") or (v = "Scalar+Inventory") or (v = "Protein+Inventory") {
                ...
                Loop, %var%
                {
                    ImageSearch, x, y, 80, 190, 670, 500, creamLib\BasicUI\x8.bmp
                    If (ErrorLevel = 0) {
                        Click, %x%, %y%, Down
                        Click, %v%, 555, Up
                        v := v + 70
	...
	Loop,
	{
		...
		ImageSearch,,, 125, 135, 140, 150, *30 creamLib\BasicUI\Hunger.bmp
		If (ErrorLevel = 1) { 
			Return "Success"
		}
...
MOUSEOVER() {
    If (A_Gui != "2")
        Return
    WinGetPos, X, Y, W, H, Vivace's Status Window 
    WinGetPos,,,, _h_, ahk_class Shell_TrayWnd
    ArrayPosition := [(A_ScreenWidth)-(W),(A_ScreenHeight)-(H+_h_)]
...
runmacro:
    ...
    gui, show,, Vivace's Stam Macro 
    WinWaitClose, Vivace's Stam Macro
